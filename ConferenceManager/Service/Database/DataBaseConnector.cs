...
namespace ConferenceManager.Service.Database {
    public class DataBaseConnector {

        static Dictionary<Type, FbDbType> _typeConvertDict = new Dictionary<Type, FbDbType>(){
            ...
            {typeof(Int16),FbDbType.SmallInt},
            {typeof(Double),FbDbType.Double}
        };

        private static FbConnection getFbdConnection() {
            var connectionString = new FbConnectionStringBuilder("ServerType=1;User=SYSDBA;Password=masterkey;Charset=UTF8;Dialect=3;Database=ConferenceManager.fbd");
        ...
        private static FbDbType ConvertType(Type type) {
            if (_typeConvertDict.ContainsKey(type))
                return _typeConvertDict[type];
